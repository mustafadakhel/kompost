 ...
 * The identifier is generated using a combination of the fragment's class canonical name,
 * tag (if available), and ID (if available) to ensure stable identification across
 * configuration changes and fragment recreations.
 *
 * This approach ensures that:
 * - Fragments of the same class but different instances have different IDs
 * - Fragment recreation (e.g., configuration change) with same tag/ID maintains same farm
 * - No memory leaks from using unstable hashCode()
 */
internal val Fragment.farmId: String
    get() {
        val className = this::class.java.canonicalName ?: this::class.java.name
        val identifier = when {
            // Prefer tag if available (set by developer, most stable)
            tag != null -> tag
            // Fall back to view ID if available
            id != 0 && id != -1 -> id.toString()
            // Last resort: use hashCode (but warn about potential issues)
            else -> {
                kompostLogger.log("WARNING: Fragment $className has no tag or ID, using hashCode. This may cause issues during configuration changes.")
                this.hashCode().toString()
            }
        }
        return "$FragmentScopedFarmName.$className.$identifier"
    }
